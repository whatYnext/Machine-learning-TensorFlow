...
Hightlights:
    1. loss functions for generator and discriminator
    2. different parameters for generator and discriminator
    3. upsampling for generator network
    4. classical newtwork for discriminator network
...
class Generator(Model):
    ...
    # Set forward pass with upsampling 
    def call(self, x, is_training=False):
        x = self.fc1(x)
        x = self.bn1(x, training=is_training) #batch is different from training and prediction
...
def discriminator_loss(disc_fake, disc_real):
    disc_loss_real = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=disc_real, labels=tf.ones([batch_size], dtype=tf.int32)))  #real loss (one as label)
    disc_loss_fake = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=disc_fake, labels=tf.zeros([batch_size], dtype=tf.int32))) #fake loss (zero as label)
...
# Train both discriminator and generator once 
